...
public class ClassLoaderFactory implements EventListenerFactoryImpl {
    private static final String HANDLER_DESC = Type.getInternalName(EventListenerImpl.class);
    ...
    @Override
    public EventListenerImpl create(Method method, Object target) throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException, ClassNotFoundException {
        Class<?> cls = createWrapper(method);
        if (Modifier.isStatic(method.getModifiers()))
            return (EventListenerImpl)cls.getDeclaredConstructor().newInstance();
        else
            return (EventListenerImpl)cls.getConstructor(Object.class).newInstance(target);
    }
