...
import org.portinglab.fabricatedeventbus.api.*;
...
public class EventBus implements EventExceptionHandlerImpl, EventBusImpl {
    ...
    private ConcurrentHashMap<Object, List<EventListenerImpl>> listeners = new ConcurrentHashMap<>();
    private final int busID = maxID.getAndIncrement();
    private final EventExceptionHandlerImpl exceptionHandler;
    private volatile boolean shutdown = false;

    private final Class<?> baseType;
    private final boolean checkTypesOnDispatch;
    private final EventListenerFactoryImpl factory;
    ...
    private EventBus(final EventExceptionHandlerImpl handler, boolean trackPhase, boolean startShutdown, Class<?> baseType, boolean checkTypesOnDispatch, EventListenerFactoryImpl factory) {
        ListenerList.resize(busID + 1);
    ...
    private <T extends Event> Class<T> getEventClass(Consumer<T> consumer) {
        ...
        if ((Class<?>)eventClass == TypeResolver.Unknown.class) {
            FabricatedForgeEventBus.LOGGER.error(FabricatedForgeEventBus.EVENTBUS_MARKER, "Failed to resolve handler for \"{}\"", consumer.toString());
    ...
    private <T extends Event> void addListener(final EventPriority priority, final Predicate<? super T> filter, final Consumer<T> consumer) {
        ...
        if (Objects.equals(eventClass, Event.class))
            FabricatedForgeEventBus.LOGGER.warn(FabricatedForgeEventBus.EVENTBUS_MARKER,"Attempting to add a Lambda listener with computed generic type of Event. " +
    ...
    {
        try {
            final ASMEventHandler asm = new ASMEventHandler(this.factory, target, method, GenericEventImpl.class.isAssignableFrom(eventType));
            ...
            FabricatedForgeEventBus.LOGGER.error(FabricatedForgeEventBus.EVENTBUS_MARKER,"Error registering event handler: {} {}", eventType, method, e);
        }
    }

    private void addToListeners(final Object target, final Class<?> eventType, final EventListenerImpl listener, final EventPriority priority) {
        ListenerList listenerList = EventListenerHelper.getListenerList(eventType);
        listenerList.register(busID, priority, listener);
        List<EventListenerImpl> others = listeners.computeIfAbsent(target, k -> Collections.synchronizedList(new ArrayList<>()));
        others.add(listener);
    }

    @Override
    public void unregister(Object object) {
        List<EventListenerImpl> list = listeners.remove(object);
        if(list == null)
            return;
        for (EventListenerImpl listener : list) {
    ...
    public boolean post(Event event) {
        return post(event, (EventListenerImpl::invoke));
    }

    @Override
    public boolean post(Event event, EventBusInvokeDispatcherImpl wrapper) {
        ...
        EventListenerImpl[] listeners = event.getListenerList().getListeners(busID);
        int index = 0;
        try {
    ...
    @Override
    public void handleException(EventBusImpl bus, Event event, EventListenerImpl[] listeners, int index, Throwable throwable) {
        LogManager.getLogger().error(FabricatedForgeEventBus.EVENTBUS, ()->new EventBusErrorMessage(event, index, listeners, throwable));
    }

    @Override
    public void shutdown() {
        LogManager.getLogger().fatal(FabricatedForgeEventBus.EVENTBUS, "EventBus {} shutting down - future events will not be posted.", busID, new Exception("stacktrace"));
        this.shutdown = true;
    }
