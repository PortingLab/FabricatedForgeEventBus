...
public class ASMEventHandler implements EventListenerImpl {

    private final EventListenerFactoryImpl factory;
    private final EventListenerImpl handler;
    private final SubscribeEvent subInfo;
    private String readable;
    private Type filter = null;

    public ASMEventHandler(EventListenerFactoryImpl factory, Object target, Method method, boolean isGeneric) throws IllegalAccessException, InstantiationException, NoSuchMethodException, InvocationTargetException, ClassNotFoundException {
    ...
    public void invoke(Event event) {
        if (handler != null) {
            if (!event.isCancelable() || !event.isCanceled() || subInfo.receiveCanceled()) {
                if (filter == null || filter == ((GenericEventImpl)event).getGenericType()) {
