...
public class EventSubclassTransformer {
    public Optional<ClassNode> transform(final ClassNode classNode, final Type classType) {
        try {
            if (Names.EVENT.equals(classType.getDescriptor())) transformEvent(classNode);
            else if (!buildEvents(classNode)) return Optional.empty();
        }
        catch (Exception e) {
            FabricatedForgeEventBus.LOGGER.error(FabricatedForgeEventBus.EVENTBUS_MARKER, "An error occurred building event handler", e);
        }
    ...
    private void transformEvent(ClassNode cls) throws Exception {
        for (MethodNode method : cls.methods) {
            ...
            if (Names.CANCELABLE_M.equals(method) || Names.HAS_RESULT_M.equals(method)) {
                clear(method);
    ...
    private boolean buildEvents(ClassNode classNode) throws Exception {
        ...
        try {
            ...
            FabricatedForgeEventBus.LOGGER.error(FabricatedForgeEventBus.EVENTBUS_MARKER, "Could not find parent {} for class {} in classloader {} on thread {}", classNode.superName, classNode.name, Thread.currentThread().getContextClassLoader(), Thread.currentThread());
            throw e;
        }
        ...
        FabricatedForgeEventBus.LOGGER.debug(FabricatedForgeEventBus.EVENTBUS_MARKER, "Event transform begin: {}", classNode.name);
        ...
        for (MethodNode method : classNode.methods) {
            if ((method.access & ACC_PUBLIC) == ACC_PUBLIC) {
                     if (Names.LISTENER_LIST_GET.equals(method)) hasGetListenerList = true;
                else if (Names.CANCELABLE_M.equals(method))      hasCancelable = true;
                else if (Names.HAS_RESULT_M.equals(method))      hasResult = true;
            }
            if (Names.INIT_M.equals(method)) hasDefaultCtr = true;
        }

        if (classNode.visibleAnnotations != null) {
            for (AnnotationNode node : classNode.visibleAnnotations) {
                if (!hasResult && node.desc.equals(Names.HAS_RESULT)) {
                    /* Add:
                     ...
                     *      }
                     */
                    MethodNode method = new MethodNode(ACC_PUBLIC, Names.HAS_RESULT_M.name(), Names.HAS_RESULT_M.desc(), null, null);
                    method.instructions.add(new InsnNode(ICONST_1));
                    method.instructions.add(new InsnNode(IRETURN));
                    classNode.methods.add(method);
                    hasResult = true;
                }
                else if (!hasCancelable && node.desc.equals(Names.CANCELLABLE)) {
                    /* Add:
                     ...
                     *      }
                     */
                    MethodNode method = new MethodNode(ACC_PUBLIC, Names.CANCELABLE_M.name(), Names.CANCELABLE_M.desc(), null, null);
                    method.instructions.add(new InsnNode(ICONST_1));
                    method.instructions.add(new InsnNode(IRETURN));
                    classNode.methods.add(method);
                    hasCancelable = true;
         ...
         */
        if (!hasDefaultCtr) {
            MethodNode method = new MethodNode(ACC_PUBLIC, Names.INIT_M.name(), Names.INIT_M.desc(), null, null);
            method.instructions.add(new VarInsnNode(ALOAD, 0));
            method.instructions.add(new MethodInsnNode(INVOKESPECIAL, tSuper.getInternalName(), Names.INIT_M.name(), Names.INIT_M.desc(), false));
            method.instructions.add(new InsnNode(RETURN));
            classNode.methods.add(method);
        }

        if (hasGetListenerList) {
            FabricatedForgeEventBus.LOGGER.debug(FabricatedForgeEventBus.EVENTBUS_MARKER, "Transforming event complete - already done: {}", classNode.name);
            return true;
        }
    ...
    private boolean addListenerList(ClassNode classNode, boolean useSuper) {
        Type tList   = Type.getType(Names.LISTENER_LIST);
        Type tHelper = Type.getType(Names.LISTENER_LIST_HELPER);
        Type tThis   = getObjectType(classNode.name);

        /* Add:
         *     private static final ListenerList LISTENER_LIST;
         */
        classNode.fields.add(new FieldNode(ACC_PRIVATE | ACC_STATIC | ACC_FINAL, Names.LISTENER_LIST_F.name(), Names.LISTENER_LIST_F.desc(), null, null));
        ...
        if (useSuper) {
            ...
            clinit.add(new MethodInsnNode(INVOKESPECIAL, tList.getInternalName(), Names.LISTENER_LIST_INIT.name(), Names.LISTENER_LIST_INIT.desc(), false));
        } else {
            // lst = new ListenerList()
            clinit.add(new MethodInsnNode(INVOKESPECIAL, tList.getInternalName(), Names.INIT_M.name(), Names.INIT_M.desc(), false));
        }
        // LISTENER_LIST = lst
        clinit.add(new FieldInsnNode(PUTSTATIC, tThis.getInternalName(), Names.LISTENER_LIST_F.name(), Names.LISTENER_LIST_F.desc()));

        MethodNode method = classNode.methods.stream().filter(Names.STATIC_INIT_M::equals).findFirst().orElse(null);
        if (method == null) {
            method = new MethodNode(ACC_STATIC, Names.STATIC_INIT_M.name(), Names.STATIC_INIT_M.desc(), null, null);
        ...
        if (method == null) {
            method = new MethodNode(ACC_PUBLIC, Names.LISTENER_LIST_GET.name(), Names.LISTENER_LIST_GET.desc(), null, null);
            classNode.methods.add(method);
        } else {
            clear(method);
        }
        method.instructions.add(new FieldInsnNode(GETSTATIC, classNode.name, Names.LISTENER_LIST_F.name(), Names.LISTENER_LIST_F.desc()));
        method.instructions.add(new InsnNode(ARETURN));
        FabricatedForgeEventBus.LOGGER.debug(FabricatedForgeEventBus.EVENTBUS_MARKER, "Event transform complete: {}", classNode.name);
        return true;
    }
}
